...
```{r}
overlap <- rownames(dge_wt[rownames(dge_wt) %in% rownames(dge_mut),])

# Select the overlapping genes from both of the dataframes for their expression values 
d1 <- as.data.frame(res_mut_mn)
d1[overlap,2,]
```

```{r}
heatmap_data <- data_heatmap(all_norm_data, samples, res_wt_mn)
signs <- sig_gene(res_wt_mn)
figure_heatmap(heatmap_data, sigs = signs)
```

```{r}
heatmap_data <- data_heatmap(all_norm_data, samples, res_mut_mn)
signs <- sig_gene(res_mut_mn)
figure_heatmap(heatmap_data, sigs = signs)
```

# Generate a heamap of the dataset above here
```{r}
heatmap_data <- data_heatmap(all_norm_data, samples, mutation_only)
signs <- sig_gene(mutation_only)
figure_heatmap(heatmap_data, sigs = signs)
```
